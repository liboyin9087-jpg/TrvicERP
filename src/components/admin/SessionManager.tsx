...
import { cn } from "@/lib/utils"; // Assuming this utility is still valid
...
export interface SessionManagerConfig {
  ...
  onCreateGroup: (
    newGroupData: Partial<TourSession>,
  ) => Promise<GroupListItem | void> | void; // Callback for creating a new group
  onUpdateGroup: (
    id: string,
    updatedGroupData: Partial<TourSession>,
  ) => Promise<GroupListItem | void> | void; // Callback for updating a group
...
function DashboardTab({
  ...
  const pendingWelfare = groups.filter(
    (g) =>
      g.group_type === "welfare" &&
      g.pending_welfare_count &&
      g.pending_welfare_count > 0,
  );
  const totalPendingWelfare = pendingWelfare.reduce(
    (sum, g) => sum + (g.pending_welfare_count || 0),
    0,
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          label="福委團數量"
          value={groups
            .filter((g) => g.group_type === "welfare")
            .length.toString()}
        />
        <StatCard
          icon={<AlertCircle className="w-5 h-5" />}
          label="待審核福委團"
          value={totalPendingWelfare.toString()}
        />
      </div>
...
function GroupRow({ group, onEdit }: GroupRowProps) {
  ...
  return (
    ...
    >
      {" "}
      {/* Make row clickable for edit/view */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center flex-wrap gap-3">
            <h4 className="font-semibold text-gray-900">
              {group.series_name || "未命名行程"}
            </h4>
            <span
              className={cn(
            ...
            {group.group_type === "welfare" && (
              <span className="px-2 py-1 bg-secondary-100 text-secondary-700 rounded-full text-sm font-semibold">
                {" "}
                {/* Using secondary token */}
                福委團
              </span>
            )}
            {group.group_number && (
        ...
        <div className="flex items-center gap-6 mt-3 sm:mt-0">
          <div className="text-right">
            ...
            <p className="text-sm text-gray-500 mt-1">
              報名進度 {group.registration_progress}%
            </p>
          </div>
...
function GroupsListTab({
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "soliciting" | "guaranteed" | "closed" | "completed"
  >("all"); // Added 'completed' status
  ...
  const filteredGroups = groups.filter((g) => {
    ...
    if (
      ...
      !g.group_number?.toLowerCase().includes(searchQuery.toLowerCase())
    )
      return false;
    return true;
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-3 items-stretch lg:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {/* Use flex-wrap for buttons */}
          {(
            ["all", "soliciting", "guaranteed", "closed", "completed"] as const
          ).map((status) => (
            <button
              key={status}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
          ...
          <table className="w-full text-sm">
            {" "}
            {/* Added text-sm for better table sizing */}
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="text-left px-4 py-3 sm:px-6 sm:py-4 text-xs sm:text-sm font-semibold text-gray-600 min-w-[150px]">
                  團體資訊
                </th>
                <th className="text-left px-4 py-3 sm:px-6 sm:py-4 text-xs sm:text-sm font-semibold text-gray-600 min-w-[80px]">
                  團型
                </th>
                <th className="text-left px-4 py-3 sm:px-6 sm:py-4 text-xs sm:text-sm font-semibold text-gray-600 min-w-[150px]">
                  日期
                </th>
                <th className="text-left px-4 py-3 sm:px-6 sm:py-4 text-xs sm:text-sm font-semibold text-gray-600 min-w-[120px]">
                  報名進度
                </th>
                <th className="text-left px-4 py-3 sm:px-6 sm:py-4 text-xs sm:text-sm font-semibold text-gray-600 min-w-[80px]">
                  狀態
                </th>
                <th className="text-left px-4 py-3 sm:px-6 sm:py-4 text-xs sm:text-sm font-semibold text-gray-600 min-w-[100px]">
                  操作
                </th>
              </tr>
            </thead>
                  ...
                  >
                      ...
                      <span
                        className={cn(
                          ...
                          group.group_type === "welfare"
                            ...
                            : "bg-gray-100 text-gray-600", // Using secondary token
                        )}
                      >
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-20 sm:w-24 h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${(group.current_pax / group.max_pax) * 100}%`,
                            }}
                          />
                        </div>
                    ...
                    <td className="px-4 py-3 sm:px-6 sm:py-4">
                      <span
                        className={cn(
                          "px-2 py-1 rounded-full text-xs sm:text-sm font-medium whitespace-nowrap",
                          group.status === "soliciting" &&
                            "bg-green-100 text-green-700",
                          group.status === "guaranteed" &&
                            "bg-primary-100 text-primary-700", // Using primary token
                          (group.status === "closed" ||
  ...
  const initialFormData: Partial<TourSession> = editingGroup
    ...
    : {
        ...
        end_date: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000)
          .toISOString()
          .split("T")[0], // 5 days later
        seat_release_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
          .toISOString()
          .split("T")[0], // 30 days later
      };

  const [formData, setFormData] =
    useState<Partial<TourSession>>(initialFormData);
...
export default function SessionManager(props: SessionManagerConfig) {
  ...
  const [editingGroupData, setEditingGroupData] =
  ...
  const handleModalSubmit = useCallback(
    async (data: Partial<TourSession>, id?: string) => {
      if (id) {
  ...
  const pendingWelfareCount = groups
    .filter(
      (g) =>
        g.group_type === "welfare" &&
        g.pending_welfare_count &&
    ...
    .reduce((sum, g) => sum + (g.pending_welfare_count || 0), 0);
