...
export default function OperationHub() {
  const getStatusStyle = (status: string) => {
    const styles: Record<string, { bg: string; text: string; label: string; icon: React.ReactNode }> = {
      on_track: { bg: 'bg-brand-100', text: 'text-brand-700', label: '正常進行', icon: <CheckCircle className="w-4 h-4" /> },
            ...
            <div key={trip.id} className="bg-white p-6 rounded-2xl border border-gray-100">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="text-right">
                  ...
                  <a href={`tel:${trip.leaderPhone}`} className="text-sm text-brand-600 flex items-center gap-1 justify-end">
