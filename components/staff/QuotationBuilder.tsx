...
export default function QuotationBuilder() {
  ...
  const [tripName, setTripName] = useState('東京五日深度遊');
  ...
  const sellingPrice = Math.round(totalCost * (1 + marginRate / 100));
  const totalRevenue = sellingPrice * paxCount;
  const profit = totalRevenue - totalCost * paxCount;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white p-6 rounded-2xl border border-gray-100">
            <h3 className="font-bold text-gray-900 mb-4">行程資訊</h3>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-100">
            <h3 className="font-bold text-gray-900 mb-4">利潤設定</h3>
            <div className="flex items-center gap-4">
              <input type="range" min="0" max="50" value={marginRate} onChange={(e) => setMarginRate(parseInt(e.target.value))} className="flex-1" />
