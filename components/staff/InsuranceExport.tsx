...
const InsuranceExport: React.FC<InsuranceExportProps> = memo(({ onExport }) => {
  ...
  const confirmedCount = useMemo(() => 
    records.filter(r => r.status === 'confirmed').reduce((sum, r) => sum + r.travelerCount, 0),
    [records]
  );
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">保險管理</h1>
          <p className="text-gray-500 mt-1">團體保險申報與匯出</p>
        </div>
        <button 
          disabled={selectedIds.length === 0} 
          onClick={handleExport}
          className="bg-black text-white px-5 py-2 rounded-xl font-semibold flex items-center gap-2 hover:bg-gray-800 transition-colors disabled:bg-gray-300"
          aria-label={`匯出 ${selectedIds.length} 筆資料`}
        >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <table className="w-full" aria-label="保險管理表格">
          <thead>
