...
const TourAddons: React.FC = memo(() => {
  ...
  const selectedItems = useMemo(() => addons.filter(a => a.selected), [addons]);
  const totalPrice = useMemo(() => selectedItems.reduce((sum, a) => sum + a.price, 0), [selectedItems]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 animate-fade-in">
      <header className="bg-black text-white px-6 py-4">
        <div className="flex items-center gap-3" aria-label="自費行程標題">
      ...
      <main className="p-6 space-y-4 pb-24">
        {addons.map((addon) => (
          <div 
            key={addon.id} 
            className={`bg-white rounded-xl border-2 overflow-hidden transition-all ${addon.selected ? 'border-brand-500' : 'border-gray-100'}`}
          ...
          >
            <div className="relative h-32">
              ...
              />
              {addon.popular && (
                <span className="absolute top-2 left-2 bg-yellow-400 text-black px-2 py-1 rounded-full text-xs font-bold flex items-center gap-1">
            ...
            <div className="p-4">
              <div className="flex items-start justify-between">
                <div>
                  <h3 id={`addon-${addon.id}-title`} className="font-bold text-gray-900">{addon.name}</h3>
                  <p className="text-sm text-gray-500 mt-1">{addon.description}</p>
                </div>
                <button 
                  onClick={() => toggleAddon(addon.id)}
                  aria-label={addon.selected ? `取消選擇 ${addon.name}` : `選擇 ${addon.name}`}
                  className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${addon.selected ? 'bg-brand-500 text-white' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
                >
      ...
      {totalPrice > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 z-40">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500">已選 {selectedItems.length} 項</p>
              <p className="text-xl font-bold text-gray-900">NT$ {totalPrice.toLocaleString()}</p>
            </div>
            <button
            ...
            {selectedItems.map(item => (
              <div key={item.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-xl">
                <div className="flex items-center gap-3">
                  <img src={item.image} alt={item.name} className="w-12 h-12 rounded-lg object-cover" aria-hidden="true" />
                  <div>
