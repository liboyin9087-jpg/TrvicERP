...
const SeasonFilter: React.FC<SeasonFilterProps> = memo(({ value, onChange, className }) => {
  const seasons: SeasonType[] = ['all', 'spring', 'summer', 'autumn', 'winter'];

  return (
    <div className={cn('flex gap-1.5 flex-wrap', className)} role="radiogroup" aria-label="季節篩選">
      {seasons.map((season) => {
