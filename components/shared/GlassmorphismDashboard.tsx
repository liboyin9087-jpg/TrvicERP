...
const GlassmorphismDashboard: React.FC<GlassmorphismDashboardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* ====== 側邊欄 ====== */}
      {sidebar && (
        <aside className="fixed left-0 top-0 h-full w-64 border-r border-white/[0.05] bg-black/30 backdrop-blur-xl z-40">
          {/* 側邊欄頂部高光 */}
      ...
      <main className={cn("relative z-0", sidebar && "pl-64")}>
        {/* Header */}
        {header && (
          <header className="sticky top-0 z-30 border-b border-white/[0.05] bg-black/20 backdrop-blur-xl">
