...
**免費優先策略**：優先使用 Google Gemini 2.5 Flash 免費模式，降低運營成本。
...
```bash
# 執行多 API 智能修正迴圈
./scripts/multi-api-ralph-loop.sh 3 ./src/components
...
│
├── scripts/
│   ├── multi-api-ralph-loop.sh   # 多 API Ralph Loop (免費優先)
...
```bash
./scripts/multi-api-ralph-loop.sh [最大迭代次數] [目標目錄]
...
- **免費優先**：優先使用 Google Gemini 2.5 Flash
- **自動備援**：SiliconFlow 作為付費備援
- **智能切換**：API 限流時自動切換
- **成本控制**：大幅降低 AI 使用成本
...
1. **使用多 API 模式**：`./scripts/multi-api-ralph-loop.sh` 會自動切換 API
2. **Gemini 免費配額**：每分鐘 15 次請求，每日 1500 次
3. **SiliconFlow 備援**：付費服務，更高的限流額度
...
{
  "scripts": {
    "validate": "./scripts/multi-api-ralph-loop.sh ./src/components",
    "validate:batch": "./scripts/batch-validate.sh ./src/components",
...
```bash
npx husky add .husky/pre-commit "./scripts/multi-api-ralph-loop.sh 1 ./src/components"
```
...
### 免費額度（Gemini 2.5 Flash）

- **免費配額**：每日 1500 次請求
- **限制**：每分鐘 15 次請求
- **適用情境**：日常開發、小型專案

### 付費備援（SiliconFlow DeepSeek V3）
...
- **理想情況**：100% 使用免費 Gemini = **$0**
- **混合使用**：70% Gemini + 30% SiliconFlow = 每月約 **$0.15 - $0.60**
- **原始成本**：100% SiliconFlow = 每月約 **$0.50 - $2.00**

**節省幅度**：最高可節省 **100%** 的 AI API 成本
...
1. **第一優先**：Google Gemini 2.5 Flash (免費)
2. **自動備援**：SiliconFlow DeepSeek V3 (付費)
3. **智能切換**：配額用完或限流時自動切換
...
📊 API 使用統計:
  🆓 Gemini 呼叫: 12 次
  💰 SiliconFlow 呼叫: 3 次
💰 總成本: ~$0 (Gemini免費) + ~$0.0015 (SiliconFlow)
```

### 故障轉移機制

- **Gemini 配額用完**：自動切換到 SiliconFlow
- **SiliconFlow 限流**：等待重試，然後回復到 Gemini
- **兩者都不可用**：報告錯誤，建議稍後重試
