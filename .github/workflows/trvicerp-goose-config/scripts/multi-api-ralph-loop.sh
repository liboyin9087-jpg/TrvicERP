...
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  TrvicERP UI 多 API 智能修正 - Ralph Loop v3${NC}"
...
/workspaces/TrvicERP/.venv/bin/pip install google-generativeai --quiet || echo "SDK已存在"
...
class MultiAPIFixer:
    ...
    def validate_and_fix_component(self, file_path):
        ...
        # 2. 使用 AI 快速驗證
        prompt = f"""
請快速檢查以下 React 元件的 TrvicERP 設計系統合規性：
...
for i in $(seq 1 $MAX_ITERATIONS); do
    ...
    if /workspaces/TrvicERP/.venv/bin/python /tmp/multi_api_fixer.py "$COMPONENT_DIR" "$GEMINI_API_KEY" "$SILICONFLOW_API_KEY" "$SILICONFLOW_API_BASE"; then
        echo ""
