...
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  TrvicERP UI 驗證 - 簡化版 Ralph Loop${NC}"
...
def validate_component(file_path, api_key, api_base):
    ...
    prompt = f"""
請分析以下 React TypeScript 元件是否符合 TrvicERP 設計系統規範：
...
for i in $(seq 1 $MAX_ITERATIONS); do
    ...
    if /workspaces/TrvicERP/.venv/bin/python /tmp/ui_validator.py "$COMPONENT_DIR" "$API_KEY" "$API_BASE"; then
        echo ""
