# TravelMaster Goose 自動化工具

## 使用 Goose + 多重 API 提供者進行 UI 元件驗證與自動修正

支援 SiliconFlow (DeepSeek V3) + Google Gemini 2.5 Flash 雙重 API 備援

本工具套件提供完整的自動化解決方案，用於驗證 TravelMaster UI 元件是否符合設計系統規範，並能自動修正發現的問題。專為 8GB RAM 低記憶體環境優化，適合小型開發團隊使用。
...
│
├── tokens/
│   └── trvic-tokens.json        # TravelMaster 設計代碼定義
│
└── utils/
...
本工具為 TravelMaster 專案內部使用，相關設計規範與代碼為專案專屬資產。
