...
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  TravelMaster UI 智能修正 - Ralph Loop v3${NC}"
...
class SmartUIFixer:
    ...
    def validate_and_fix_component(self, file_path):
        ...
        # 1. 快速驗證（使用 SF 或 Groq）
        validation_prompt = f"""驗證 TravelMaster 設計規範：
{content[:3000]}
...
"""
        ...
        # 2. 使用 Gemini 進行實際修正
        if issues_found:
            fix_prompt = f"""修正 TravelMaster 元件設計問題：
...
for i in $(seq 1 $MAX_ITERATIONS); do
    ...
    if /workspaces/TravelMaster/.venv/bin/python /tmp/smart_fixer.py "$COMPONENT_DIR" "$SILICONFLOW_API_KEY" "$SILICONFLOW_API_BASE" "$SILICONFLOW_MODEL" "$GROQ_API_KEY" "$GROQ_API_BASE" "$GROQ_MODEL" "$GEMINI_API_KEY"; then
        echo ""
